...
$query = "SELECT id, tweet_id, text, lang, datetime FROM climate_tweets ct WHERE lang IN ( ".implode(',', $langArray).") AND NOT EXISTS(SELECT * FROM tweet_classifications tc WHERE tc.tweet_id != $tweet_id and tc.user_id != $user_id) ORDER BY RAND() LIMIT 1";
error_log($query);
